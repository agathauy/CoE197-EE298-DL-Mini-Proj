...
OUTPUT_FOLDER = './output/midi_bts_single_boy_with_luv_v1'

#INPUT_FOLDER = 'input/stardew_valley_overture'
#OUTPUT_FOLDER = 'output/stardew_valley_overture'
...
def create_network(network_input, n_vocab):
    ...
    model = Sequential()
    model.add(CuDNNLSTM(1024,input_shape=(network_input.shape[1], network_input.shape[2]),return_sequences=True))
    model.add(Dropout(0.3))
    model.add(Bidirectional(CuDNNLSTM(1024, return_sequences=True)))
    model.add(Dropout(0.3))
    model.add(Bidirectional(CuDNNLSTM(1024)))
    model.add(Dense(256))
    model.add(Dropout(0.3))
