...
def midi_to_array_one_hot(mid, quantization, PITCHES_TO_INDEX):
    ...
    for (position, note_type, note_num, velocity) in notes:
        ...
        if note_type == "note_on":
            ...
            midi_array[position, 3*PITCHES_TO_INDEX[note_num]+2] = 80
...
def unquantize_track(orig_track, style_track):
    ...
    orig_cum_msgs.sort(key=lambda cum_time_msg: cum_time_msg[0])
    style_cum_msgs.sort(key=lambda cum_time_msg1: cum_time_msg1[0])
...
def quantize_track(track, ticks_per_quarter, quantization):
    ...
    quantized_msgs.sort(
        key=lambda cum_time_msg2: cum_time_msg2[0]
        if (cum_time_msg2[1].type=='note_on' and cum_time_msg2[1].velocity > 0) else cum_time_msg2[0] + 0.5)
...
def velocity_range(mid):
    ...
    dynamics = len(list(velocities.keys()))
    # print velocities
    if dynamics > 1:
        return dynamics
    else:
